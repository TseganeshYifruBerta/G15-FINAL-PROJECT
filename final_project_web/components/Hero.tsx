...
import { fetchInfoQuestion, fetchInfoUsers } from "@/store/info/info_Api";
...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userResponse = await fetchInfoUsers();
        const problemResponse = await fetchInfoQuestion();

        console.log('User response:', userResponse);
        console.log('Problem response:', problemResponse);

        setListUser(prevListUser => [
          { ...prevListUser[0], number: userResponse.count.toString() },
          prevListUser[1], // Institutions (remains unchanged)
          { ...prevListUser[2], number: problemResponse.count.toString() }
        ]);
      } catch (error) {
            ...
            >
              <div className="flex mx-auto w-40 sm:w-auto">
                <div className="flex items-center justify-center bg-[#d4d7fa] w-12 h-12 mr-6 rounded-full">
                  <Image src={listUsers.icon} className="h-6 w-6" alt="picture" width={52} height={53} />
                </div>
