...
const UpcomingExams = () => {
  ...
  // Function to convert examTime and examDate to a timestamp
  const convertExamTimeToTimestamp = (examTime: string, examDate: string): number => {
  ...
  useEffect(() => {
    if (upcomingExams) {
      const examTimestamp = convertExamTimeToTimestamp(upcomingExams.examDate, upcomingExams.examTime);
      if (examTimestamp > Date.now()) {
        const timerInterval = setInterval(() => {
          const currentTime = new Date(); 
          const differenceInMilliseconds = examTimestamp - currentTime.getTime();
          ...
          <div className="flex items-center space-x-4 h-full">
            ...
            <div className="flex flex-col justify-between h-full gap-4">
              ...
              />
                  ...
                  <div className={`text-xs border-2 border-primary p-2 m-2 rounded-md ${timerColor}`}>
