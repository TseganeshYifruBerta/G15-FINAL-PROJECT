  ...
  return (
    <div className="bg-gray-100 rounded-xl text-gray-500 drop-shadow-sm">
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <tbody>
          <tr key={examId} className="bg-white text-left text-left text-gray-500 odd:bg-primary odd:bg-opacity-5">
            <td className="py-3 pl-3">
              <Link href={`/plagiarism/${examId}`}>
                <p className="dark:text-white">{title}</p>
              </Link>
            </td>
            <td className="py-3 pr-8 hidden sm:table-cell">
              <Link href={`/plagiarism/${examId}`}>
